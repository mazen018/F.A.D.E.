...
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	if (command === `play`) {
		...
		if (!voiceChannel) return msg.channel.send('have to be in a vc.');
		const permissions = voiceChannel.permissionsFor(msg.client.user);
		if (!permissions.has('CONNECT')) {
			//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
			return msg.channel.send('can not speak here');
		}//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
...
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
					...
					try {
						...
						console.error(err);
						return msg.channel.send('no song has chosen');
					}
...
${prefix}skip ⇏ skip da song what is playing now
${prefix}pause ⇏ stop any song for a short time
${prefix}resume ⇏ for open it again
${prefix}vol ⇏  100 - 0
${prefix}stop ⇏ make da bot leave
${prefix}np ⇏ what song is playing now? 
${prefix}queue ⇏ لمعرفة قآئمة التشغيل
 ...
 `)//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
   message.channel.sendEmbed(embed)//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	...
	client.login("NDUyMTY5NTM5MDExMjE1Mzg1.DfMbTg.LOOZlB79LFDEshEDltGfdh6gn8g");
